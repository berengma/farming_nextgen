...
local S = farmingNG.S
	...
	farmingNG.harvester_names["farming:barley_8"]	= true
	farmingNG.harvester_names["farming:barley_7"]	= farmingNG.harvester_nofullg
...
local function recursive_harvest(pos, remaining_charge)
	...
	for npos in farmingNG.iterSawTries(pos) do
		...
		if farmingNG.harvester_names[core.get_node(npos).name] then
...
if farmingNG.havetech then
	if technic.plus then
		...
		core.register_tool("farming_nextgen:harvester", {
			description = S("Harvester"),
			inventory_image = "farming_nextgen_harvester.png",
			stack_max = 1,
